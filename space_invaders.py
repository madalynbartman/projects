...
def main():
    ...
    def redraw_window():
        # 0,0 is the top left in pygame so y=0 is the very top of the screen
        WIN.blit(BG, (0,0))
        # draw white text with lives & level
        ...
        for enemy in enemies:
            enemy.draw(WIN) # no draw method in enemies. It inherits it from ship.

        player.draw(WIN) # If overlapping, player will be on top of enemy cause it's drawn second
    ...
    while run:
        ...
        if len(enemies) == 0:
            ...
            for i in range(wave_length):
                enemy = Enemy(random.randint(50, WIDTH - GREEN_SPACESHIP.get_width() - 50), random.randint(-1500, -100), random.choice(["red", "blue", "green"]))
