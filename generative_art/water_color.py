# start off with a base octagon and 
# layer it on top of itself dozens of times
# deforming the edge each time
# pycairo allows you to draw vector images
...
colors = []
for i in range(15):
    colors.append((float_gen(.4, .75), float_gen(.4, .75), float_gen(.4, .75))) # avoid pure blakc and pure white
...
def main():
    ...
    # algo of deforming & layering
    for p in range(-int(height*.2), int(height*1.2), 60):
        cr.set_source_rgba(random.choice(colors)[0], random.choice(colors)[2], shapealpha)

        # want it to retain some shape
        # if you deform it too much, your final layers will vary wildly
